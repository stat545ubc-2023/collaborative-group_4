...
``` r
### ERROR HERE ### 
# Used the transmute() function instead of mutate()
transmute(movieLens,
...
### ERROR HERE ###
movieLens %>%
  count(year) # use count() function, which acts as a "group_by" and "tally" 2-in-1
```
...
### ERROR HERE ###
movieLens %>% 
  count(title, rating, sort = TRUE) # checked ?count() to see c() function not necessary
```
...
``` r
### ERROR HERE ###
# Replaced the mutate() function defining the min and max rating (applied to whole data set)
# Used group_by to group data by movie title, then applied summarize() to define new columns for the min and max rating per movie
...
starWars %>%
  group_by(species) %>%
  summarise(across(all_of(c("height", "mass")), ~ min(.x, na.rm = TRUE)))
```

    ## Warning: There were 6 warnings in `summarise()`.
    ## The first warning was:
    ## ℹ In argument: `across(all_of(c("height", "mass")), ~min(.x, na.rm = TRUE))`.
...
fakeStarWars <- tribble(
  ~name,            ~birth_year, ~birth_location,
  "Luke Skywalker",  1998,        "Liverpool, England",
  "C-3PO"         ,  1999,        "Liverpool, England",
  "R2-D2"         ,  2000,        "Seattle, WA",
  "Darth Vader"   ,  2001,        "Liverpool, England",
  "Leia Organa"   ,  2002,        "New York, NY",
  "Owen Lars"     ,  2003,        "Seattle, WA",
  "Beru Whitesun Iars", 2004,     "Liverpool, England",
  "R5-D4"         ,  2005,        "New York, NY"
) %>%
  mutate(birth_weight = (birth_year - 1995) * 0.45)
```
